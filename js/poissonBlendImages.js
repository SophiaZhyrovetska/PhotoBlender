...
function sinmpleBlend(newcanvas, srccanvas, mask_data, finalcanvas, refImage, base_size){
    ...
    for (let i = 0; i <base_size.width*base_size.height*4-4 ; i++) {
        if(mask_pixels.data[i]===0 && mask_pixels.data[i+1]===255 &&
...
//partly from: https://github.com/takuti/poisson-image-blending
...
function poissonBlendImages(newcanvas, srccanvas, mask_data, finalcanvas, refImage, base_size){
    ...
    do {
        ...
        for(let y=1; y<base_size.height-1; y++) {
            for(let x=1; x<base_size.width-1; x++) {
                ...
                if(mask_pixels.data[p+0]===0 && mask_pixels.data[p+1]===255 &&
                    ...
                    let neighbours = [((y-1)*base_size.width+x)*4, ((y+1)*base_size.width+x)*4,
                        (y*base_size.width+(x-1))*4, (y*base_size.width+(x+1))*4];
                    let num_neighbors = neighbours.length;
                    ...
                    for(let rgb=0; rgb<3; rgb++) {
                        ...
                        for(let i=0; i<num_neighbors; i++) {
                            let q_offseted = neighbours[i] + 4*(blend_position_offset.y*base_size.width+blend_position_offset.x);

                            if(mask_pixels.data[neighbours[i]]===0 && mask_pixels.data[neighbours[i]+1]===255 &&
                                mask_pixels.data[neighbours[i]+2]===0 && mask_pixels.data[neighbours[i]+3]===255) {
                            ...
                            if(is_mixing_gradients && Math.abs(base_pixels.data[p_offseted+rgb]-base_pixels.data[q_offseted+rgb]) >
                                Math.abs(src_pixels.data[p+rgb]-src_pixels.data[neighbours[i]+rgb])) {
                                sum_vpq += base_pixels.data[p_offseted+rgb]-base_pixels.data[q_offseted+rgb];
                            } else {
                                sum_vpq += src_pixels.data[p+rgb]-src_pixels.data[neighbours[i]+rgb];
                            }
                        }
        ...
        cnt++;
        let epsilon = dx/absx;
        if(!epsilon || previous_epsilon-epsilon === 0) break;
